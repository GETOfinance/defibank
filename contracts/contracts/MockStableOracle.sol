...
/// @notice Simple owner-settable oracle: localPerUSDC in 1e18 per symbol
contract MockStableOracle is IStableOracle, Ownable {
    mapping(bytes32 => uint256) public price; // local per USDC, 1e18

    constructor(address owner_) Ownable(owner_) {}

    function localPerUSDC(bytes32 symbol) external view override returns (uint256) {
        return price[symbol];
    }
