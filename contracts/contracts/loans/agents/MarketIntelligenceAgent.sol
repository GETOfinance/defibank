    ...
    AccessControl
{
    ...
    // Helper to read price from Hub
    function _getPriceFromHub(address token) internal view returns (uint256 price, uint256 timestamp, uint256 volatility) {
        (bool ok, bytes memory data) = hubContract.staticcall(abi.encodeWithSignature("getPrice(address)", token));
        if (!ok || data.length == 0) return (0, 0, 0);
