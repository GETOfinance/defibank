    ...
    AccessControl
{
    ...
    // State variables
    // Price helper to read from hub
    function _getPriceFromHub(address token) internal view returns (uint256) {
        (bool ok, bytes memory data) = hubContract.staticcall(abi.encodeWithSignature("getPrice(address)", token));
        if (!ok || data.length == 0) return 0;
        (uint256 price,,) = abi.decode(data, (uint256, uint256, uint256));
        return price;
    }
    ...
    function _getCollateralValue(address token, uint256 amount) internal view returns (uint256) {
        ...
        uint256 price = _getPriceFromHub(token);
