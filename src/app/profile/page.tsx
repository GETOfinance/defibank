...
const potStatusColors: { [key: number]: string } = {
  0: 'bg-orange-500/20 text-orange-500',
...
export default function ProfilePage() {
    ...
    <motion.button
      ...
      className={`
        ...
        ${active
          ...
          : 'bg-black/40 backdrop-blur-xl border border-orange-500/20 text-gray-400 hover:text-white'
        }
      `}
    ...
    >
      ...
      {count !== undefined && count > 0 && (
        <span className={`
          ...
          ${active ? 'bg-black/20' : 'bg-orange-500/20'}
        `}>
          {count}
        </span>
      )}
      {active && (
        <motion.div
          className="absolute inset-0 bg-gradient-to-r from-orange-500/20 to-orange-600/20 rounded-xl blur-lg -z-10"
  ...
  const StatsCard: React.FC<{
    ...
    <motion.div
      ...
      onClick={onClick}
    >
      <div className="absolute inset-0 bg-gradient-to-r from-orange-500/10 to-orange-600/10 rounded-xl blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <div className="relative bg-black/40 backdrop-blur-xl p-6 rounded-xl border border-orange-500/20 group-hover:border-orange-500/40 transition-all duration-300 h-full">
        <div className="flex justify-between items-start mb-4">
          <div className="text-orange-400 p-2 bg-orange-500/10 rounded-lg">
            {icon}
          </div>
          {percentage !== undefined && (
            <div className={`text-sm ${percentage >= 0 ? 'text-orange-400' : 'text-red-400'}`}>
  ...
  const EmptyState: React.FC<EmptyStateProps> = ({ icon, message, actionLabel, onAction }) => (
    <motion.div
      ...
      variants={fadeIn}
    >
      <div className="text-orange-400/40 mb-4 flex justify-center">{icon}</div>
      <p className="text-gray-400 mb-4">{message}</p>
      {actionLabel && onAction && (
        <motion.button
          onClick={onAction}
          className="inline-flex items-center space-x-2 px-4 py-2 bg-orange-500/10 rounded-lg text-orange-400 hover:bg-orange-500/20 transition-colors"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
        >
