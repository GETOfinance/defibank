...
export default function SavingsPotsPage() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-gray-900 via-black to-orange-950">
      ...
      >
        ...
        <motion.div variants={itemAnimation} className="text-center mb-16 relative">
          ...
          <motion.div
            ...
            className="inline-block mb-6 bg-black/30 p-6 rounded-2xl backdrop-blur-xl border border-orange-500/10"
          >
            <CurrencyDollarIcon className="w-16 h-16 mx-auto text-orange-400" />
          </motion.div>
          
          <h1 className="text-6xl font-bold mb-6 relative">
            <span className="bg-gradient-to-r from-orange-400 via-orange-300 to-orange-500 text-transparent bg-clip-text">
              Savings Pots
            </span>
            <motion.span
          ...
          <div className="flex justify-center gap-6">
            <motion.div
              ...
              className="flex items-center space-x-3 px-6 py-3 rounded-xl bg-gradient-to-r from-orange-500/10 to-orange-600/10 backdrop-blur-xl border border-orange-500/10"
            >
              <ChartBarIcon className="w-5 h-5 text-orange-400" />
              <span className="text-orange-400 font-medium">Track Progress</span>
            </motion.div>
            <motion.div
              variants={itemAnimation}
              whileHover={{ scale: 1.05, y: -5 }}
            ...
            >
              ...
              <span className="text-orange-400 font-medium">Secure Storage</span>
            </motion.div>
          </div>
        </motion.div>
          ...
          {activeTab === 'create' ? (
            ...
            >
              <div className="relative">
                ...
                <div className="relative bg-black/40 backdrop-blur-xl p-8 rounded-2xl border border-orange-500/20">
                  <form onSubmit={handleCreatePot} className="space-y-6">
                    <div>
                      <label className="block mb-2 text-orange-400 font-medium">Pot Name</label>
                      <input
                        type="text"
                        value={name}
                        onChange={(e) => setName(e.target.value)}
                        className="w-full px-4 py-3 rounded-xl bg-black/50 border border-orange-500/20 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500/40 transition-all duration-200"
                    ...
                    <div>
                      <label className="block mb-2 text-orange-400 font-medium">Target Amount ({currentChain.symbol})</label>
                      <input
                        type="number"
                        value={targetAmount}
                    ...
                    <div>
                      <label className="block mb-2 text-orange-400 font-medium">Remarks</label>
                      <textarea
                        value={remarks}
                    ...
                    {success && (
                      <motion.div 
                        className="p-4 rounded-xl bg-orange-500/10 border border-orange-500/20 text-orange-400"
                        initial={{ opacity: 0, y: -10 }}
                        animate={{ opacity: 1, y: 0 }}
                      >
                        {success}
                    ...
                    <motion.button
                      type="submit"
                      className="w-full bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 py-3 rounded-xl font-semibold flex items-center justify-center space-x-2 hover:brightness-110 disabled:opacity-50 disabled:hover:brightness-100 transition-all duration-200"
            ...
            >
              {isFetching ? (
                ...
                <motion.div 
                  className="text-center bg-black/40 backdrop-blur-xl p-12 rounded-2xl border border-orange-500/20"
                  variants={itemAnimation}
                >
...
const TabButton: React.FC<TabButtonProps> = ({ isActive, onClick, icon, text, count }) => (
  <motion.button
    ...
    className={`
      ...
      ${isActive 
        ...
        : 'bg-black/40 backdrop-blur-xl border border-orange-500/20 text-white hover:border-orange-500/40'
      }
    `}
  ...
  >
      ...
      {count !== undefined && (
        <motion.span 
          className={`
            ...
            ${isActive ? 'bg-black/20' : 'bg-orange-500/20'}
          `}
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
    ...
    {isActive && (
      <motion.div
        className="absolute inset-0 bg-gradient-to-r from-orange-500/20 to-orange-600/20 rounded-xl blur-xl -z-10"
        layoutId="activeTab"
...
const PotCard: React.FC<PotCardProps> = ({ pot, onContribute, onBreak, isLoading }) => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-r from-orange-500/10 to-orange-600/10 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

      <div className="relative bg-black/40 backdrop-blur-xl p-8 rounded-2xl border border-orange-500/20 group-hover:border-orange-500/40 transition-all duration-300">
        <div className="flex justify-between items-start mb-6">
          <div>
            <h3 className="text-xl font-semibold text-orange-400 mb-1">{pot.name}</h3>
            ...
            {showContribute ? (
              ...
              >
                ...
                <div className="flex space-x-3">
                  <motion.button
                    ...
                    className="flex-1 bg-gradient-to-r from-orange-500 to-orange-600 text-white px-4 py-3 rounded-xl font-semibold flex items-center justify-center space-x-2 hover:brightness-110 disabled:opacity-50 disabled:hover:brightness-100 transition-all duration-200"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    disabled={isLoading}
                  >
              ...
              <div className="flex space-x-3">
                <motion.button
                  ...
                  className="flex-1 bg-gradient-to-r from-orange-500 to-orange-600 text-white px-4 py-3 rounded-xl font-semibold flex items-center justify-center space-x-2 hover:brightness-110 transition-all duration-200"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
        ...
        {progress >= 100 && pot.status === 0 && (
          ...
          >
            <div className="bg-orange-500 text-white p-2 rounded-full shadow-lg shadow-orange-500/25">
              <CheckCircleIcon className="w-5 h-5" />
            </div>
          </motion.div>
        )}
