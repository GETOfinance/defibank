...
const ProfileQR: React.FC<ProfileQRProps> = ({ username, address, onClose }) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Pay ${username} on DeFi Bank`,
          text: `Send payment to ${username} using DeFi Bank`,
          url: `https://defibank.com/transfer?to=${address}`
        });
      } catch (err) {
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      downloadLink.download = `mimimoneypay-${username}.png`;
